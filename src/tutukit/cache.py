...
class DiskCache:
    """Дисковый кэш ответов MCP с тремя режимами работы.

    ``record`` — обычный: промах идёт в сеть, ответ ложится на диск.
    ``replay`` — только диск: демо переживает падение `mcp.tutu.ru`.
    ``off`` — без кэша, для проверки живого поведения.

    Ключ — инструмент плюс хэш аргументов, поэтому один и тот же прогон всегда
